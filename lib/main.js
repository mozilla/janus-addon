...
const PREV_DEFAULT_PROXY_URL = "http://janus.allizom.org";
const DEFAULT_PROXY_URL = "https://janus.allizom.org";
const PROXY_TYPE = 2;
...
var ProxyAddon = {
  ...
  observeAddon: function(doc, topic, id) {
    ...
    let reportBugButton = doc.getElementById("report-bug-button");
...
if (self.loadReason == 'install') {
  ...
  if (Preferences.get(PREFS.FORK_DISABLED, null) === null) {
    Preferences.set(PREFS.FORK_DISABLED, true);
  }
  if (Preferences.get(PREFS.PAC_URL, null) === PREV_DEFAULT_PROXY_URL) {
    Preferences.set(PREFS.PAC_URL, DEFAULT_PROXY_URL);
  }
}

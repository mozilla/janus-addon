...
function showDoorhanger(window) {
  let buttons = [
    /* There is no way to show the addon options on Fennec right now, it seems
    {
      label: "Options",
      callback: function () {
        window.NativeWindow.toast.show("options", "short");
      }
    },
    */
    {
  ...
  if (usage.totalEgress == 0 || usage.totalIngress == 0) {
    ...
    message = usage.totalIngress + " in / " + usage.totalEgress + " out\n" +
      usage.totalUnknown + " not proxied\n" +
      usage.reductionProxied + " saved proxied\n" +
      usage.reductionOverall + " saved overall";
  }

  window.NativeWindow.doorhanger.show(message, "janus", buttons,
                                      window.BrowserApp.selectedTab.id);
}

function onLongClick() {
  showDoorhanger(gWindows[0]);
}
...
function updateUIForWindow(window, enabled) {
  var needToast = !!gPageAction;
  if (gPageAction) {
    window.NativeWindow.pageactions.remove(gPageAction);
    gPageAction = null;
  }

  gPageAction = window.NativeWindow.pageactions.add({
...
function getIcon() {
  return Preferences.get(PREFS.ENABLED) ? JANUS_ICON_ENABLED : JANUS_ICON_DISABLED;
}

function getIconTitle() {
  return Preferences.get(PREFS.ENABLED) ? "Disable Proxy" : "Enable Proxy";
}
...
function loadIntoWindow(window) {
  ...
  updateUIForWindow(window, Preferences.get(PREFS.ENABLED));
}
